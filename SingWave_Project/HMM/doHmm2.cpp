...
// 기존 함수를 확장하여 오토인코더로 저장된 파일을 읽어오는 기능 추가
SHORT* CGestureDoc::GetCodeFromAutoencoderOutput(const std::string& fileName, SHORT* pnData) {
    std::ifstream file(fileName);
    if (!file.is_open()) {
        Warning("(GetCodeFromAutoencoderOutput) Failed to open file.");
        return nullptr;
    }
    ...
    // 관찰값의 개수 설정
    *pnData = dataVectors.size();
    SHORT* output = new SHORT[*pnData];
    for (int i = 0; i < *pnData; ++i) {
        // 여기서 벡터 데이터를 코드로 변환
        // 예를 들어, HMM의 상태 수에 맞게 벡터를 양자화하거나 변환
        output[i] = QuantizeVector(dataVectors[i]);
    }
...
// 벡터 양자화 함수 (사용자 정의)
SHORT CGestureDoc::QuantizeVector(const std::vector<float>& vector) {
    // 벡터를 HMM 코드로 변환하는 로직을 구현
    // 예: 평균값 또는 특정 상태로 매핑
    SHORT code = /* 양자화 로직 */;
    return code;
}
